...
class QuestionnaireResultExport
{
    ...
    {
        $translations = TranslationHelper::getTranslations($request->get('lang'), 'therapist_portal');
        ...
        $questionnaire = json_decode($response);
        $questions = [];
        $activities = [];
        if (!empty($questionnaire->data) && !empty($questionnaire->data->questions)) {
            $questions = $questionnaire->data->questions;
            $activities = Activity::where('type', Activity::ACTIVITY_TYPE_QUESTIONNAIRE)->where('activity_id', $questionnaire->data->id)->where('completed', 1)->get();
        }
